...
nb = input('\n How many datapoints? [400] '); 
if isempty(nb) 
    nb=400; 
end
sig = input('\n Dataset dispersion ? [0.3] '); 
if isempty(sig) 
    sig=0.3; 
end
...
nc = input('\n Number of components to be extracted? [6] '); 
if isempty(nc) 
    c=6; 
end
sig2 = input('\n RBF kernel parameter sig2? [0.4] '); 
if isempty(sig2) 
    sig2=0.4; 
end
approx = input('\n Approximation technique ? 1 for ''Lanczos'', 2 for ''Nystrom'' [1] '); 
if isempty(approx) 
    approx=1; 
end
